...
//        act.keyDown(Keys.PAGE_UP).keyUp(Keys.PAGE_UP).build().perform();
//
...
//    }
    ...
    public void navigateToBookStoreForSearch() throws InterruptedException {
...
//        js.executeScript("window.scrollBy(0,document.body.scrollHeight)");
    ...
    public void navigateToProfileAndVerifyBookAddedToCollection(String bookName) throws InterruptedException {
        ...
        if (size == 1) {
            ...
            Assert.assertTrue("Book does not exist in collection",false);
        }
    ...
    public void userLogoutFromApplication() {
      ewait().until(ExpectedConditions.elementToBeClickable(By.xpath("//button[text()='Log out']")));
        ...
        {
            Assert.assertTrue("User is not able to logout from BookStore",false);
        }
    }
